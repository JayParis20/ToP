...
void ClosestHitGBuffer(inout RayIntersectionGBuffer rayIntersectionGbuffer : SV_RayPayload, AttributeData attributeData : SV_IntersectionAttributes)
{
	...
	RT_TRANS_CO( fragInput.texCoord0.xy, _MainTex_var , RTD_TRAN_OPA_Sli , RTD_CO , bo_co_val , true, GLO_OUT);
	...
	//RT_CA
	float3 RTD_CA = RT_CA( (color + GLO_OUT) * GetCurrentExposureMultiplier() );
	...
	rayIntersectionGbuffer.gbuffer3 = float4(finalRGBA.rgb,1.0) * GetInverseCurrentExposureMultiplier();
